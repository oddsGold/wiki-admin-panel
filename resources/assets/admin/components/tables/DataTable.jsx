...
const DataTable = ({ data, gridHeaderRow, handleDelete, setEditPath, location, sort, setSort }) => (
    <Table>
                ...
                {gridHeaderRow.map((header, index) => (
                    ...
                    >
                        <div className="flex items-center justify-between cursor-pointer">
                            <b>{header.label}</b>
                            {header.sortable && (
                                <button className="flex flex-col gap-0.5" onClick={() => header.sortable && handleSort(header.name, sort, setSort)}>
                                    <svg className="text-gray-300 dark:text-gray-700  text-brand-500" width="8" height="5" viewBox="0 0 8 5" fill="none">
            ...
            {data.map((row, rowIndex) => (
                <TableRow key={row.id}>
                    {gridHeaderRow.map((header, cellIndex) => (
                        <TableCell key={cellIndex} className="px-4 py-3 text-gray-500 text-start text-theme-sm border border-gray-100 dark:text-gray-400 dark:border-white/[0.05]">
                            {header.badge ? (
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm border border-gray-100 dark:text-gray-400 dark:border-white/[0.05]">
                        <div className="flex items-center w-full gap-2">
                            <Link to={`${setEditPath}/${row.id}/edit`} state={{ from: location }} className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
                                <svg width="1em" height="1em" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" className="size-5">
                            ...
                            <button className="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500" onClick={() => handleDelete(row)}>
                                <svg width="1em" height="1em" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="size-5">
